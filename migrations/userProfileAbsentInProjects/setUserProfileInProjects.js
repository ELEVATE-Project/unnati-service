...
(async () => {
    ...
    try {
        ...
        for (let pointerToProject = 0; pointerToProject < chunkOfProjectDocument.length; pointerToProject++) {
            ...
            //get user ids of project without profile
            let userIdsWithoutProfile = await db.collection('projects').find({
                _id: { $in : projectIds }
            }).project({ 
                "_id" : 1,
                "userId" : 1
            ...
            //loop userIds- These user profiles are absent in project
            for ( let count = 0; count < userIdsWithoutProfile.length; count++ ) {

                let projectIdWithoutUserProfile = userIdsWithoutProfile[count]._id;
                let userId = userIdsWithoutProfile[count].userId;
