...
const viewFullReport = function (token, input) {
	return new Promise(async (resolve, reject) => {
		try {
			const url = reportsUrl + CONSTANTS.endpoints.VIEW_PROJECT_REPORT
...
const entityReport = function (token, input) {
	return new Promise(async (resolve, reject) => {
		try {
			const url = reportsUrl + CONSTANTS.endpoints.ENTITY_REPORT
			let options = {
				headers: {
					'internal-access-token': process.env.INTERNAL_ACCESS_TOKEN,
					'content-type': 'application/json',
...
const projectAndTaskReport = function (token, input, projectPdf) {
	console.log(token, input, projectPdf, 'line no 115')
	return new Promise(async (resolve, reject) => {
		try {
			const url = reportsUrl + CONSTANTS.endpoints.PROJECT_AND_TASK_REPORT + '?projectPdf=' + projectPdf
