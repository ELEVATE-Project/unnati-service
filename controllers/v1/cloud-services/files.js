  ...
  async upload(req) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!fs.existsSync(`${PROJECT_ROOT_DIRECTORY}/public/assets/${formattedDate}`)) {
          fs.mkdirSync(`${PROJECT_ROOT_DIRECTORY}/public/assets/${formattedDate}`);
        }
        let filename = path.basename(req.query.file);
        let localFilePath = `${PROJECT_ROOT_DIRECTORY}/public/assets/${formattedDate}/${filename}`;
        let uploadStatus
